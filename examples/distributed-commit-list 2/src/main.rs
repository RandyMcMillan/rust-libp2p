...
use libp2p::{
    ...
    kad::{
        store::MemoryStore, store::MemoryStoreConfig, Config as KademliaConfig, Mode, Record,
        RecordKey,
    },
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // TODO --key arg
    let local_keypair = libp2p::identity::Keypair::ed25519_from_bytes([0; 32]).unwrap();

    // 2. Get the public key from the key pair.
    let public_key = local_keypair.public();

    // 3. Create the PeerId from the public key.
    //    `PeerId` has a `From` trait implementation for `PublicKey`.
    ...
    // 1. Create a MemoryStoreConfig instance.
    //    You can use the default, or build it with custom settings.
    let mut memory_store_config = MemoryStoreConfig {
    ...
    // 3. Create the MemoryStore using the custom configuration.
    let store = MemoryStore::with_config(local_peer_id, memory_store_config);

    // 4. Create the Kademlia behavior using the configured store.
